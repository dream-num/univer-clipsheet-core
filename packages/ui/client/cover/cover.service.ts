...
export class CoverService {
    ...
    findCoverByElement(element: HTMLElement) {
        return Array.from(this._coverMap.values()).find((cover) => cover.target === element);
    }

    hasCover(id: string) {
        return this._coverMap.has(id);
    }

    addCover(id: string, element: HTMLElement) {
        const existedCover = this.findCoverByElement(element);
        if (existedCover) {
            this._coverMap.set(id, existedCover);
            return;
        }
